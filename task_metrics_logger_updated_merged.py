...
CSV columns:
  ...
  6) urgency
  7) priority
  8) timestamp_start_attending_human_request
  9) timestamp_end_attending_human_request
  10) robot_responding_to_human_request
  11) robot_total_task_start_time
  12) robot_total_task_end_time
"""
...
class TaskMetricsLogger(Node):
    ...
    def _finalize_human_task(self, rec: TaskRecord, raw_event_json: Optional[str]) -> None:
        ...
        # task type
        urgency_value = rec.urgency if rec.urgency not in ("", None) else (human.urgency if human else "")
        priority_value = rec.priority if rec.priority not in ("", None) else (human.priority if human else "")

        urgency_value = "" if urgency_value in (None, "") else str(urgency_value)
        priority_value = "" if priority_value in (None, "") else str(priority_value)
